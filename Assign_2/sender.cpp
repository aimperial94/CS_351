...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
    ...
    */
    ...
    shmid = shmget(key, SHARED_MEMORY_CHUNK_SIZE, S_IWUSR | S_IRUSR);
...
unsigned long sendFile(const char* fileName)
{
    ...
    while(!feof(fp))
    {
         ...
         */
        if(msgrcv(msqid, &rcvMsg, sizeof(ackMessage) - sizeof(long), 2, 0) < 0){
...
void sendFileName(const char* fileName)
{
    ...
    strcpy(fName.fileName, fileName);
    cout << "snd fName: " << fName.fileName << endl;

    /* TODO: Send the message using msgsnd */
    if(msgsnd(msqid, &fName, sizeof(message) - sizeof(long), 0) < 0){
